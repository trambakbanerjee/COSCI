...
write.csv(selected.d2,paste(wd,'/selected.csv',sep=""))

#------------------------- COSCI+SpKM and COSCI+KM --------------------------
DD<- scale(select.d2,TRUE,TRUE)

km.perm <- KMeansSparseCluster.permute(DD,K=nclass,nperms=25)
km.out <- KMeansSparseCluster(DD,K=nclass,wbounds=km.perm$bestw)
...
for (i in 1:30){
  set.seed(i^2)
  vkm<- kmeans(DD,nclass, iter.max = 50, nstart = 30,algorithm = "Hartigan-Wong")
...
dframe1<- as.data.frame(cbind((p-3):p,score.raw[order(score.raw)[(p-3):p]]))
...
g2<-ggplot(dframe1,aes(x=x,y=S_j)) + geom_point(color='blue',size=3)+
