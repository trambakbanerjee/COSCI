...
out <- matrix(rep(0,p*reps),reps,p)
for (r in 1:reps){
  ...
  grp<- d[,p+1]
  d <- d[,1:p]
  fit2 = hill_climb_GSS(d,k=96,nperms=50,itermax=50,threshold=0,tolerance=1)
  out[r,fit2$final_set]<- 1
}

FN.hill<- rowSums(out[,1:6]==0)# Signal identified as Noise
FP.hill<- rowSums(out[,7:100]>0)# Noise identified as signal
tab_200<- cbind(sum(FN.hill)/reps,sd(FN.hill)/sqrt(reps),sum(FP.hill)/reps,sd(FP.hill)/sqrt(reps))
...
FN.hill<- rowSums(out[,1:6]==0)# Signal identified as Noise
FP.hill<- rowSums(out[,7:100]>0)# Noise identified as signal
tab_1000<- cbind(sum(FN.hill)/reps,sd(FN.hill)/sqrt(reps),sum(FP.hill)/reps,sd(FP.hill)/sqrt(reps))
...
FN.hill<- rowSums(out[,1:6]==0)# Signal identified as Noise
FP.hill<- rowSums(out[,7:100]>0)# Noise identified as signal
tab_2500<- cbind(sum(FN.hill)/reps,sd(FN.hill)/sqrt(reps),sum(FP.hill)/reps,sd(FP.hill)/sqrt(reps))
