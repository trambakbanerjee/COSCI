...
for (r in 1:reps){
  ...
  grp<- d[,p+1]
  d <- d[,1:p]
  fit2 = hill_climb_GSS(d,k=32,nperms=50,itermax=50,threshold=0,tolerance=1)
  out[r,fit2$final_set]<- 1
}

FN.hill<- rowSums(out[,1:5]==0)# Signal identified as Noise
FP.hill<- rowSums(out[,6:50]>0)# Noise identified as signal
