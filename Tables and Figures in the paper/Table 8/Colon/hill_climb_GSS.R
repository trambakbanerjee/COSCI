...
#------------------------------------------------------
setwd(wd)
d<- as.matrix(read.csv('colon_x.txt',header=F,sep=' '))
true.class<- as.matrix(read.csv('colon_y.txt',header=F,sep=' '))
...
out <- matrix(0,1,p)
fit2 = hill_climb_GSS(tdata,k=nclass,nperms=50,itermax=20,threshold=0,tolerance=0)
