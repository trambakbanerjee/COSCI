...
#------------------------------------------------------
setwd(wd)
d1<- as.matrix(read.csv('lung_gordon1_x.txt',header=F,sep=''))
d2<- as.matrix(read.csv('lung_gordon2_x.txt',header=F,sep=''))
d3<- as.matrix(read.csv('lung_gordon3_x.txt',header=F,sep=''))
d4<- as.matrix(read.csv('lung_gordon4_x.txt',header=F,sep=''))
d<- cbind(d1,d2,d3,d4)
true.class<- as.matrix(c(rep(1,150),rep(2,31)))
...
out <- matrix(0,1,p)
fit2 = hill_climb_GSS(tdata,k=nclass,nperms=25,itermax=20,threshold=0,tolerance=0)
