...
setwd(wd)
d<- as.matrix(read.csv('lymphoma_x.txt',header=F,sep=' '))
true.class<- as.matrix(read.csv('lymphoma_y.txt',header=F,sep=' '))
...
see<- order(x)
interval<- c(0,x[see[ceiling(0.85*p)]])
xi<- x[x<=interval[2]]
N0<- length(xi)
N<- length(x)
theta<- (N0/N)
...
out <- mle2(nll,start=list(a=4,b=5),data=list(n0=N0,u=interval[2]))
...
km.perm <- KMeansSparseCluster.permute(D,K=nclass,wbounds=seq(1.1,7,len=15),nperms=10)
km.out <- KMeansSparseCluster(D,K=nclass,wbounds=km.perm$bestw)
